...
int dns_tcp_connect(void) {
    if (dns_mode_none()) { debugf("dns_tcp_connect: disabled by mode none"); errno = ENOSYS; return -1; }
...
static int should_force_loopback_dns53(const struct sockaddr *addr) {
    return loopback_dns_force_enabled() && is_loopback_addr(addr) && sockaddr_is_dns53(addr);
}

static int should_force_any_dns53(const struct sockaddr *addr) {
    if (dns_mode_none()) return 0;
    if (should_force_loopback_dns53(addr)) return 1;
    if (!dns_mode_full()) return 0;
...
static int replace_fd(int fd, int manager_fd, int kind) {
    ...
    if (real_fcntl_fn) {
        if (fl >= 0) real_fcntl_fn(fd, F_SETFL, fl);
        if (fdfl >= 0) real_fcntl_fn(fd, F_SETFD, fdfl);
    }
...
int connect(int fd, const struct sockaddr *addr, socklen_t len) {
    init_real();
    if (debug_enabled() && addr && (addr->sa_family == AF_INET || addr->sa_family == AF_INET6)) {
        char ip[INET6_ADDRSTRLEN] = {0}; uint16_t port = 0; int family = 0;
        if (sockaddr_to_ip_port(addr, ip, sizeof(ip), &port, &family) == 0) debugf("connect fd=%d family=%d type=%d ip=%s port=%u", fd, family, fd_ok(fd)?tracked[fd].type:0, ip, (unsigned)port);
    }
    if (fd_ok(fd) && tracked[fd].active && addr && (addr->sa_family == AF_INET || addr->sa_family == AF_INET6)) {
        if ((tracked[fd].type & SOCK_TYPE_MASK) == SOCK_STREAM && should_force_any_dns53(addr)) {
            return force_dns_stream_fd(fd);
        }
        if ((tracked[fd].type & SOCK_TYPE_MASK) == SOCK_DGRAM && should_force_any_dns53(addr)) {
...
ssize_t recvfrom(int fd, void *buf, size_t len, int flags, struct sockaddr *src, socklen_t *srclen) {
    ...
    if (src && srclen) {
        ...
        int fam = tracked[fd].remote_family ?
                  tracked[fd].remote_family :
                  tracked[fd].domain;

        fill_sockaddr_from_text(
            fam,
            tracked[fd].remote_ip,
            tracked[fd].remote_port,
            src,
            srclen
        );
    }
...
static int fill_sockaddr_from_text(int family, const char *ip, uint16_t port,
    ...
    if (!addr) {
        *addrlen = family == AF_INET6 ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in);
        return 0;
    }
    if (family == AF_INET6) {
        struct sockaddr_in6 in6;
...
static int managed_getsockname(int fd, struct sockaddr *addr, socklen_t *addrlen) {
    if (!fd_ok(fd) || !tracked[fd].proxied) {
        errno = EBADF;
        return -1;
    }
    if (tracked[fd].kind == KIND_TCP_STREAM || tracked[fd].kind == KIND_TCP_LISTENER ||
        tracked[fd].kind == KIND_UDP_CONNECTED || tracked[fd].kind == KIND_UDP_LISTENER) {
        int family = tracked[fd].bound ? tracked[fd].bind_family :
                     (tracked[fd].domain == AF_INET6 ? AF_INET6 : AF_INET);
        const char *ip = tracked[fd].bound ? tracked[fd].bind_ip :
...
static int managed_getpeername(int fd, struct sockaddr *addr, socklen_t *addrlen) {
    if (!fd_ok(fd) || !tracked[fd].proxied) {
        errno = ENOTCONN;
        return -1;
    }
    if (tracked[fd].kind == KIND_TCP_STREAM || tracked[fd].kind == KIND_UDP_CONNECTED) {
        if (!tracked[fd].remote_port) {
            errno = ENOTCONN;
            return -1;
        }
        return fill_sockaddr_from_text(
            tracked[fd].remote_family ? tracked[fd].remote_family :
            (tracked[fd].domain == AF_INET6 ? AF_INET6 : AF_INET),
            tracked[fd].remote_ip,
            tracked[fd].remote_port,
            addr,
...
int fcntl(int fd, int cmd, ...) {
    ...
    if (fd_ok(fd) && tracked[fd].proxied) {
        switch (cmd) {
        case F_GETFL:
            return tracked[fd].saved_fl ? tracked[fd].saved_fl : (real_fcntl_fn ? real_fcntl_fn(fd, cmd) : 0);
        case F_SETFL:
            tracked[fd].saved_fl = (int)arg;
            return real_fcntl_fn ? real_fcntl_fn(fd, cmd, arg) : 0;
        case F_GETFD:
            return tracked[fd].saved_fdfl ? tracked[fd].saved_fdfl : (real_fcntl_fn ? real_fcntl_fn(fd, cmd) : 0);
        case F_SETFD:
...
int getsockopt(int fd, int level, int optname, void *optval, socklen_t *optlen) {
    ...
    if (fd_ok(fd) && tracked[fd].proxied && optval && optlen) {
        if (level == SOL_SOCKET) {
            if (optname == SO_ERROR && *optlen >= sizeof(int)) { *(int *)optval = 0; *optlen = sizeof(int); return 0; }
            if (optname == SO_TYPE && *optlen >= sizeof(int)) { *(int *)optval = tracked[fd].type & SOCK_TYPE_MASK; *optlen = sizeof(int); return 0; }
#ifdef SO_DOMAIN
            if (optname == SO_DOMAIN && *optlen >= sizeof(int)) { *(int *)optval = tracked[fd].remote_family ? tracked[fd].remote_family : tracked[fd].domain; *optlen = sizeof(int); return 0; }
#endif
#ifdef SO_PROTOCOL
            if (optname == SO_PROTOCOL && *optlen >= sizeof(int)) { *(int *)optval = tracked[fd].protocol; *optlen = sizeof(int); return 0; }
#endif
#ifdef SO_ACCEPTCONN
            if (optname == SO_ACCEPTCONN && *optlen >= sizeof(int)) { *(int *)optval = tracked[fd].kind == KIND_TCP_LISTENER; *optlen = sizeof(int); return 0; }
#endif
        }
#ifdef IPPROTO_TCP
#ifdef TCP_NODELAY
        if (level == IPPROTO_TCP && optname == TCP_NODELAY && *optlen >= sizeof(int)) { *(int *)optval = 1; *optlen = sizeof(int); return 0; }
#endif
#endif
    }
    return real_getsockopt_fn ? real_getsockopt_fn(fd, level, optname, optval, optlen) : -1;
}
...
int setsockopt(int fd, int level, int optname, const void *optval, socklen_t optlen) {
    ...
    return real_setsockopt_fn ? real_setsockopt_fn(fd, level, optname, optval, optlen) : -1;
}
...
ssize_t sendmsg(int fd, const struct msghdr *msg, int flags) {
    ...
    if (fd_ok(fd) && tracked[fd].proxied) {
        size_t len = 0, off = 0;
        for (size_t i = 0; i < msg->msg_iovlen; i++) len += msg->msg_iov[i].iov_len;
        ...
        ssize_t r;
        if ((tracked[fd].kind == KIND_UDP_LISTENER || tracked[fd].kind == KIND_UDP_CONNECTED) && msg->msg_name) {
            r = sendto(fd, buf, len, flags, (const struct sockaddr *)msg->msg_name, msg->msg_namelen);
        } else {
...
ssize_t recvmsg(int fd, struct msghdr *msg, int flags) {
    ...
    if (!real_recvmsg_fn) real_recvmsg_fn = dlsym(RTLD_NEXT, "recvmsg");
    init_real();
    if (fd_ok(fd) && tracked[fd].proxied) {
        char buf[65536];
        ssize_t r;
        ...
        socklen_t sl = sizeof(ss);
            ...
            if (r > 0 && msg->msg_name) {
                socklen_t copy = msg->msg_namelen < sl ?
                         msg->msg_namelen : sl;
        ...
        for (size_t i = 0; i < msg->msg_iovlen && off < (size_t)r; i++) {
            ...
            if (c > (size_t)r - off) c = (size_t)r - off;
...
int recvmmsg(int fd, struct mmsghdr *vmessages, unsigned int vlen, int flags, struct timespec *timeout) {
    static int (*real_recvmmsg_fn)(int, struct mmsghdr *, unsigned int, int, struct timespec *) = NULL;
    if (!real_recvmmsg_fn) real_recvmmsg_fn = dlsym(RTLD_NEXT, "recvmmsg");
    init_real();
    (void)timeout;
    if (fd_ok(fd) && tracked[fd].proxied) {
        unsigned int i;
    ...
    return real_recvmmsg_fn ? real_recvmmsg_fn(fd, vmessages, vlen, flags, timeout) : -1;
}
